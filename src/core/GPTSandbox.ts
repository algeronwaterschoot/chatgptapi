...
import { GPTCodeBlockError } from "./Errors"




export async function deprecatedExtractCodeFromChatReply(arr: string[]) {
...
export function extractCodeBlocks(input: string): string[] {
    ...
    for (let inputLine of inputLines) {
        if (inputLine.includes('```')) inputLine = '```'
    ...
    if (count % 2 !== 0) {
        throw new GPTCodeBlockError('Incomplete code block detected. ChatGPT probably ran out of buffer in its answer.')
    }
    ...
    if (inputLines.length < 3) throw new Error('Unexpected error #2 while processing ChatGPT code block')
    // - Every even-numbered entry is a code block.
    // - The total number of entries should be uneven.
    if (inputLines.length % 2 === 0) throw new Error('Unexpected error #3 while processing ChatGPT code block')
...
async function extractJavaScriptCode(text: string) {
...
async function extractJavaScriptCode2(text: string) {
...
export const isNodeJs = (typeof window === 'undefined') // 'Running in Node.js'
...
//@ts-ignore
const sandboxBrowser = async (code: string) => { const result = await executeJavaScript(code) }
const sandboxNodeJS = (code: string) => {
    const sandbox = {
...
export const sandbox = isNodeJs ? sandboxNodeJS : sandboxBrowser

export async function sandboxEval(code: string): Promise<{ success: boolean; output: any }> {
    try { await sandbox(code) } catch (error: any) { return { success: false, output: error.message } }
